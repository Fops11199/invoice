    ...
    >
        ...
        <div className="flex items-center justify-between">
          <label className="flex items-center gap-2 cursor-pointer select-none group">
            <input
              ...
              className="h-4 w-4 rounded border-slate-300 text-primary focus:ring-primary/30 cursor-pointer accent-[#1a56db]"
            />
      ...
      <div className="mt-6">
        ...
        <div className="mt-4 space-y-3">
          {/* Google GSI renders here if VITE_GOOGLE_CLIENT_ID is set */}
          <div id="google-signin-btn" className="flex justify-center"></div>

          {/* Fallback Google button when VITE_GOOGLE_CLIENT_ID is not configured */}
          {!import.meta.env.VITE_GOOGLE_CLIENT_ID && (
            <button
              type="button"
              onClick={() => alert("To enable Google Sign-In, add VITE_GOOGLE_CLIENT_ID to your frontend .env and GOOGLE_CLIENT_ID to your backend .env file.\n\nGet one from: console.cloud.google.com → APIs & Services → Credentials → OAuth 2.0 Client ID")}
              className="w-full flex items-center justify-center gap-3 rounded-full border border-slate-300 bg-white px-5 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-all shadow-sm"
            >
